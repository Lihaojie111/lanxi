...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'money', 'mobile', 'avatar', 'score','first_leader'];
     ...
     * @param string $username 用户的微信昵称
     * @param string $avatar 用户的微信头像
     * @param string $gender 性别
     * @param array  $extend   扩展参数
     * @return boolean
     */
    public function register($openid, $username, $avatar, $gender, $extend = [])
    {
        // 检测用户名或邮箱、手机号是否存在
        ...
        if (User::getByWxopenid($openid)) {
            $this->setError('此微信号已注册，请勿重复注册');
            return false;
        }
        $ip = request()->ip();
        $time = time();
